...
abstract class ViewBuilder[RM <: ReadModelObject : ClassTag] extends MovieServiceManager with ElasticsearchSupport{
    ...
    Flow[EventEnvelope].
      ...
      collect{
        ...
        case EnvelopeAndAction(env, u:UpdateAction) =>
          log.info("The event seq is {}",env.sequenceNr)
  ...
  def updateDocumentField(id:String, seq:Long, expressions:List[String], params:Map[String,Any]):Future[IndexingResult] = {
    ...
    log.info("The version is {}",seq)
    val request = UpdateRequest(UpdateScript(script))
