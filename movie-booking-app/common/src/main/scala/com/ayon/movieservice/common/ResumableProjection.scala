...
class CassandraProjectionStorageExt(system:ActorSystem) extends Extension {
  ...
  val createKeyspaceStmt = """
      CREATE KEYSPACE IF NOT EXISTS moviebookinginfo
      WITH REPLICATION = { 'class' : 'SimpleStrategy', 'replication_factor' : 1 }
    """

  val createTableStmt = """
      CREATE TABLE IF NOT EXISTS moviebookinginfo.projectionoffsets (
  ...
  def updateOffset(identifier:String, offset:Long): Future[Boolean] = (for {
    ...
    _ <- session.executeAsync(s"update moviebookinginfo.projectionoffsets set offset = $offset where identifier = '$identifier'")
  } yield true) recover { case t => false }

  def fetchLatestOffset(identifier:String): Future[Option[Long]] = for {
    session <- session.underlying()
    rs <- session.executeAsync(s"select offset from moviebookinginfo.projectionoffsets where identifier = '$identifier'")
  } yield {
