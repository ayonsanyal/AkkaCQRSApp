 ...
 * Marker trait  that is an event generated as the result of a command
 */
 ...
 */
object MovieServicePersistentEntity{
 ...
 */
abstract class MovieServicePersistentEntity[FO <: EntityFieldsObject[String, FO]: ClassTag](id:String)
  extends PersistentActor with ActorLogging{
  import  MovieServicePersistentEntity._
  ...
  def isAcceptingCommand(cmd:Any):Boolean = {
    log.info("creation {} command is accepted with state {} and cmd {} ", isCreateMessage(cmd),state,cmd)
  ...
  def stateResponse(respectDeleted:Boolean = true) = 
    ...
    //Otherwise, return it as a CompleteResult that is just persisted/

    else CompleteResult(state)
  ...
  def handleEventAndRespond(respectDeleted:Boolean = true)(event:EntityEvent):Unit = {
    ...
    log.info("sender is {}",sender().path)
 ...
 */
abstract class Aggregate[FO <: EntityFieldsObject[String, FO], E <: MovieServicePersistentEntity[FO] : ClassTag] extends MovieServiceManager{
